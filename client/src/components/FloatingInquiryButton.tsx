...
export function FloatingInquiryButton({ onOpen }: FloatingInquiryButtonProps) {
  return (
    <div className="fixed bottom-5 right-5 z-40 flex flex-col items-end gap-2 sm:bottom-7 sm:right-7">
      ...
      <a href={barunfoodLocation.phoneHref} aria-label={`${barunfoodLocation.phone}로 전화 상담하기`} className="inline-flex items-center gap-2 border border-[#d9d0c2] bg-[#fffdf8] px-3 py-2 text-[11px] font-bold tracking-[0.04em] text-[#242321] shadow-[0_8px_20px_rgba(36,35,33,0.14)] transition duration-200 hover:-translate-y-0.5 hover:border-[#9e3328] hover:text-[#9e3328] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#9e3328]">
        <PhoneCall className="h-3.5 w-3.5" aria-hidden="true" />
        <span>{barunfoodLocation.phone}</span>
      </a>
