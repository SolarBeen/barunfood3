      ...
      <div className="relative max-h-[92vh] w-full max-w-2xl overflow-y-auto bg-[#f7f3eb] shadow-[0_28px_80px_rgba(20,18,14,0.32)]">
        ...
        {submitted ? (
          ...
          <form onSubmit={submitInquiry} noValidate className="p-7 sm:p-10"><p className="text-xs font-bold tracking-[0.2em] text-[#9e3328]">PARTNERSHIP DESK</p><h2 id="inquiry-title" className="mt-3 pr-8 font-serif text-3xl font-semibold tracking-[-0.055em] text-[#242321] sm:text-4xl">필요한 조건부터<br />함께 검토합니다.</h2><p className="mt-4 text-sm leading-6 text-[#655f56]">문의 유형을 선택하고, 검토에 필요한 기본 정보를 남겨주세요.</p>{Object.keys(errors).length > 0 && <div role="alert" className="mt-6 flex items-center gap-3 border-l-2 border-[#9e3328] bg-[#f0e2dc] px-4 py-3 text-sm text-[#6f2c24]"><X className="h-4 w-4 shrink-0" />입력 내용을 다시 확인해 주세요.</div>}{sendError && <div role="alert" className="mt-4 border-l-2 border-[#9e3328] bg-[#f0e2dc] px-4 py-3 text-sm leading-6 text-[#6f2c24]">{sendError}</div>}
            ...
            <div className="mt-7 grid gap-5 sm:grid-cols-2"><label className="block text-xs font-bold tracking-[0.12em] text-[#4e4942]">회사명 또는 성함<span className="ml-1 text-[#9e3328]">*</span><input value={values.name} onChange={(e) => updateValue("name", e.target.value)} onBlur={() => handleBlur("name")} aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "name-error" : undefined} className={inputClass("name")} placeholder="바른푸드" /></label><Error id="name-error" message={errors.name} /><label className="block text-xs font-bold tracking-[0.12em] text-[#4e4942]">연락처<span className="ml-1 text-[#9e3328]">*</span><input value={values.phone} onChange={(e) => updateValue("phone", e.target.value)} onBlur={() => handleBlur("phone")} inputMode="tel" aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone ? "phone-error" : undefined} className={inputClass("phone")} placeholder="010-0000-0000" /></label><Error id="phone-error" message={errors.phone} /><label className="block text-xs font-bold tracking-[0.12em] text-[#4e4942]">이메일<span className="ml-1 text-[#9e3328]">*</span><input value={values.email} onChange={(e) => updateValue("email", e.target.value)} onBlur={() => handleBlur("email")} type="email" aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? "email-error" : undefined} className={inputClass("email")} placeholder="name@company.com" /></label><Error id="email-error" message={errors.email} /><label className="block text-xs font-bold tracking-[0.12em] text-[#4e4942]">관심 제품군<input value={values.product} onChange={(e) => updateValue("product", e.target.value)} className={inputClass("product")} placeholder="도우, 피자 등" /></label></div>
            <label className="mt-5 block text-xs font-bold tracking-[0.12em] text-[#4e4942]">문의 내용<span className="ml-1 text-[#9e3328]">*</span><textarea value={values.message} onChange={(e) => updateValue("message", e.target.value)} onBlur={() => handleBlur("message")} aria-invalid={Boolean(errors.message)} aria-describedby={errors.message ? "message-error" : undefined} className={`${inputClass("message")} min-h-28 resize-none`} placeholder="검토가 필요한 조건을 10자 이상 남겨주세요." /></label><Error id="message-error" message={errors.message} />
            <label className="mt-5 flex items-start gap-2 text-xs leading-5 text-[#655f56]"><input checked={values.consent} onChange={(e) => updateValue("consent", e.target.checked)} onBlur={() => handleBlur("consent")} type="checkbox" aria-invalid={Boolean(errors.consent)} aria-describedby={errors.consent ? "consent-error" : undefined} className="mt-0.5 accent-[#9e3328]" /><span>문의 응대를 위한 개인정보 수집·이용에 동의합니다.<span className="ml-1 text-[#9e3328]">*</span></span></label><Error id="consent-error" message={errors.consent} />
...
function Error({ id, message }: { id: string; message?: string }) {
  ...
  return <p id={id} className="-mt-3 text-[11px] leading-4 text-[#9e3328]" role="alert">{message}</p>;
}
